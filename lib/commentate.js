...
module.exports = () => {
  ...
  imgur.getUserSub().then(posts => {
    ...
    eachSeries(posts, (post, cb) => {
      ...
      if ((post.title).includes("cat") && LastPost != post.id){
            LastPost = post.id
            makeComment2(post, cb)            
      }      
      else if ((post.title).includes("dog") && LastPost != post.id || (post.title).includes("pup") && LastPost != post.id){
            LastPost = post.id
            makeComment3(post, cb)            
      }      
      else if (post.title == "." && LastPost != post.id){
            LastPost = post.id
            makeComment(post, cb)            
      }
    }, done => {
