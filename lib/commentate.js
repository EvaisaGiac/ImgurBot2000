...
module.exports = () => {
  ...
  imgur.getUserSub().then(posts => {
    ...
    eachSeries(posts, (post, cb) => {
      ...
      if ((post.title).includes("dog") && LastPost != post.id || (post.title).includes("golden") && LastPost != post.id || (post.title).includes("pup") && LastPost != post.id || (post.title).includes("corgi") && LastPost != post.id || (post.title).includes("shib") && LastPost != post.id || (post.title).includes("husky") && LastPost != post.id){
            LastPost = post.id
            makeComment3(post, cb)            
      }      
      if ((post.title).length < 5 && LastPost != post.id){
            LastPost = post.id
            makeComment(post, cb)            
      }
    }, done => {
