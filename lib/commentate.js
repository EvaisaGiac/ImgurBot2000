...
module.exports = () => {
  ...
  imgur.getUserSub().then(posts => {
    ...
    eachSeries(posts, (post, cb) => {
      ...
      winston.info(`Checked ${post.title} length is ${(post.title).length} comparing to items: ${(a)}`)
      
      if ((post.title).includes("cat") && a.indexOf(post.id) || (post.title).includes("kitty") && a.indexOf(post.id)){
            LastPost = post.id
            a.push(post.id)
            makeComment2(post, cb)            
      }      
      else if ((post.title).includes("dog") && a.indexOf(post.id) || (post.title).includes("golden") && a.indexOf(post.id) || (post.title).includes("pup") && a.indexOf(post.id) || (post.title).includes("corgi") && a.indexOf(post.id) || (post.title).includes("shib") && a.indexOf(post.id) || (post.title).includes("husky") && a.indexOf(post.id)){
            LastPost = post.id
            a.push(post.id)
            makeComment3(post, cb)            
      }      
      else if ((post.title).length < 5 && a.indexOf(post.id)){
            LastPost = post.id
            a.push(post.id)
            makeComment(post, cb)            
      }
